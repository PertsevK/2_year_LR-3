...
def matrix5(filename):
    """Reading a matrix from a file, calculating parameters and performing an operation on the matrix"""
    M = N = K = 0
    ...
    try:
        ...
        if result is not None:
            ...
            for i, (mean, min_element) in enumerate(zip(means, min_elements), start=1):
                print(f"Column {2*i-1}: Arithmetic mean = {mean}, Minimum element = {min_element}")
